...
function importer (archive, src, opts, cb) {
  ...
  const ignoreDirs = false;//!(opts.ignoreDirs === false)
  ...
  if (debug.enabled) {
    progress.on('put', function (src, dst) {
      console.log('IMPORT put:', dst.name)
    })
    progress.on('del', function (file) {
      console.log('IMPORT del:', file.name)
    })
    progress.on('ignore', function (src, dst) {
      console.log('IMPORT ignore:', dst.name)
    })
    progress.on('skip', function (src, dst) {
      console.log('IMPORT skip:', dst.name)
    })
